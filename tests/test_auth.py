...
from movensense.server.auth import get_or_create_token, TOKEN_FILE


class TestTokenAuth:
    def test_generates_32_char_hex_token(self, tmp_path):
        token_file = tmp_path / "token"
        with patch("movensense.server.auth.TOKEN_FILE", token_file):
            with patch("movensense.server.auth.CONFIG_DIR", tmp_path):
