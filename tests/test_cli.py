...
from movesense.cli import cli, _load_env_serial, _resolve_serials
...
class TestEnvLoading:
    def test_load_msn_from_env_file(self, tmp_path):
        ...
        with patch("movesense.cli.Path.cwd", return_value=tmp_path):
    ...
    def test_load_msn_from_parent_env(self, tmp_path):
        ...
        with patch("movesense.cli.Path.cwd", return_value=child):
            result = _load_env_serial()
        assert result == "999"
...
class TestCLICommands:
    ...
    def test_status_dispatches(self, runner):
        with patch("movesense.cli._run") as mock_run:
    ...
    def test_fetch_edf_flag(self, runner):
        with patch("movesense.cli._run") as mock_run:
            ...
            with patch("movesense.cli._output_dir", return_value=Path("/tmp")):
